...
public interface EventApi {
    ...
    Event singleEvent(@PathParam("roomId") String roomId, @PathParam("eventId") String eventId);
     ...
     * @param from   Required. The token to start returning events from. This token can be obtained from a prev_batch token returned for
     *               each room by the sync API, or from a start or end token returned by a previous request to this endpoint.
     * @param to     The token to stop returning events at. This token can be obtained from a prev_batch token returned for each room by the
     *               sync endpoint, or from a start or end token returned by a previous request to this endpoint.
     ...
     * @param txnId   Required. The transaction ID for this event. Clients should generate a unique ID; it will be used by the server
